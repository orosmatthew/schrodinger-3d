    ...
    for (const DeferredOperation& deferred : m_deferred_operations) {
        if (const auto deferred_copy = std::get_if<DeferredCopyStagingBuffer>(&deferred)) {
            ...
            cmd_copy_buffer_to_image(
                ...
                deferred_image->dst_image.depth);
...
void Renderer::update_texture(const Texture& texture, const std::byte* data)
{
    MVE_VAL_ASSERT(m_textures.contains(texture.handle()), "[Renderer] Invalid texture")
    constexpr uint32_t mip_levels = 1;
    // ReSharper disable once CppUseStructuredBinding
    ...
    m_deferred_operations.emplace_back(
        DeferredCopyBufferImage { mip_levels, staging_buffer, texture_impl.image, texture_impl.image.vk_format });
}
