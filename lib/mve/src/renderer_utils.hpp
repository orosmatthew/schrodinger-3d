    ...
    const bool dedicated)
{
    ...
    return { vk::Image(image), image_allocation, width, height, depth, format };
}
    ...
    const vk::Format image_format)
{
    ...
    std::ranges::transform(swapchain_images, std::back_inserter(image_views), [&](const vk::Image swapchain_image) {
        return create_image_view(
            loader, device, swapchain_image, image_format, vk::ImageAspectFlagBits::eColor, 1, vk::ImageViewType::e2D);
    });
